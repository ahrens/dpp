...
{
	zfs_ioctl_arg_t *zia = pthread_getspecific(pid_key);
	int as_fd;

	char *as_path;
	asprintf(&as_path, "/proc/%u/as", zia->zia_pid);
	as_fd = open(as_path, O_RDWR);
	free(as_path);
	if (as_fd == -1) {
		perror("open as");
		return (EFAULT);
	}
	if (pread(as_fd, dst, size, (uintptr_t)src) != size) {
		perror("pread as");
		return (EFAULT);
	}
	close(as_fd);
...
copyinstr(const char *src, char *dst, size_t size, size_t *copied)
{
	...
	if (copied != NULL)
		*copied = 0;
	do {
		if (pread(as_fd, dst, 1, (uintptr_t)src) != 1) {
			perror("pread as");
			return (EFAULT);
		}
		src++;
...
int
ddi_copyout(const void *src, void *dst, size_t size, int flag)
{
	int error;
	zfs_ioctl_msg_t msg = { 0 };

	printf("copyout %u bytes to %p\n", size, dst);

	msg.zim_type = ZIM_COPYOUT;
	msg.zim_u.zim_copyin.zim_address = (uintptr_t)dst;
	msg.zim_u.zim_copyin.zim_len = size;
	...
	if (msg.zim_type != ZIM_COPYOUT_RESPONSE)
		return (EINVAL);
	printf("errno=%u\n", msg.zim_u.zim_copyin_response.zim_errno);
	return (msg.zim_u.zim_copyin_response.zim_errno != 0);
}
...
ddi_copyout(const void *src, void *dst, size_t size, int flag)
{
	...
	if (pwrite(as_fd, src, size, (uintptr_t)dst) != size) {
		perror("pwrite as");
		return (EFAULT);
	}
	close(as_fd);
...
zfs_user_ioctl_init(void)
{
	...
	pthread_create(NULL, NULL, accept_connection,
